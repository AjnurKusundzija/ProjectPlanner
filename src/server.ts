...
server.registerTool(
    'create_project',
    {
        ...
        inputSchema: {
            project_name: z.string().min(1, 'Ime projekta ne može biti prazno'),
            deadline: z
                .string()
                .min(1)
...
server.registerTool(
    'update_project',
    {
        ...
        inputSchema: {
            ...
            project_name: z.string().min(1).optional().describe('Novo ime projekta'),
            deadline: z
                .string()
                .min(1)
                .optional()
...
server.registerTool(
    'manage_todo',
    {
        ...
        inputSchema: {
            ...
            project_id: z.number().int().positive(),
            todo_id: z.number().int().positive().optional().describe('Potrebno za toggle i delete'),
            text: z.string().min(1).optional().describe('Tekst — potrebno za add'),
        },
    },
...
server.registerResource(
    ...
    async (uri: URL) => {
        ...
        const todos = data.projects.flatMap((p) =>
            p.todolist.map((t) => ({
                todo_id: t.id,
                text: t.text,
                is_done: t.is_done,
...
server.registerPrompt(
    'analyze_project',
    { argsSchema: { project_name: z.string().min(1) } },
    async ({ project_name }) => {
        const data = readDB()
...
server.registerPrompt(
    ...
    async ({ project_name }) => {
        ...
        const existingTodos =
            project && project.todolist.length > 0
...
Generiši 4-6 konkretnih todos stavki vezanih za projekat.`,
        ...
        let parsed: { project_name: string; deadline: string; todos: string[] }
