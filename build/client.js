...
const FRONTEND_DIR = path_1.default.resolve('frontend');
...
function getProject(args) {
    ...
    if (!project_name)
        return { success: false, message: 'project_name je obavezan.' };
    const data = readDB();
    const project = data.projects.find((p) => p.project_name.trim().toLowerCase() === project_name);
    if (!project)
        return { success: false, message: 'Trazeni projekat nije pronadjen.' };
...
function updateProject(args) {
    ...
    if (!project)
        return { success: false, message: `Projekat sa ID ${project_id} nije pronadjen.` };
    if (project_name)
...
function manageTodo(args) {
    ...
    if (action === 'add') {
        if (!text)
            return { success: false, message: 'Za add je potreban text.' };
        const todo = { id: nextId(project.todolist), text, is_done: false };
    ...
    if (!todo)
        return { success: false, message: `Todo ID ${todo_id} nije pronadjen.` };
    if (action === 'toggle') {
...
function addTodoToProject(args) {
    const project_name = String(args.project_name ?? '').trim().toLowerCase();
...
function buildDailyOverviewPrompt() {
    const data = readDB();
...
function buildAnalyzeProjectPrompt(args) {
    const project_name = String(args.project_name ?? '').trim();
    if (!project_name)
        return 'project_name je obavezan argument za analyze_project.';
    const data = readDB();
    const project = data.projects.find((p) => p.project_name.toLowerCase() === project_name.toLowerCase());
    if (!project)
        return `Projekat "${project_name}" nije pronadjen u sistemu.`;
...
function buildSuggestTodosPrompt(args) {
    const project_name = String(args.project_name ?? '').trim();
    if (!project_name)
        return 'project_name je obavezan argument za suggest_todos.';
    const data = readDB();
    const project = data.projects.find((p) => p.project_name.toLowerCase() === project_name.toLowerCase());
...
function extractToolCallsFromFailedGeneration(failedGeneration) {
    const calls = [];
...
function summarizeToolRecovery(results) {
    if (results.length === 0) {
...
function recoverFromToolUseFailure(err) {
    const maybeErr = err;
...
function handleDeterministicChatTool(message) {
    const normalized = message.toLowerCase();
    const wantsDeleteAll = (normalized.includes('obrisi sve projekte') ||
...
const prompts = [
    {
        ...
        arguments: [],
        run: (_args) => buildDailyOverviewPrompt(),
    },
    {
        ...
        run: (args) => buildAnalyzeProjectPrompt(args),
    },
    {
        name: 'suggest_todos',
        ...
        run: (args) => buildSuggestTodosPrompt(args),
    },
];
